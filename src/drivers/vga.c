...
void drawRect(multiboot_info_t *mbi, int x, int y, int w, int h, int r, int g, int b){ //Draw a filled rectangle
	int i, j;
...
void drawText(multiboot_info_t *mbi, int charnum){//draw a single character using a 8*8 pattern
	...
	if (height == 720) { // scrolling screen by clearing
		clearScreen(mbi); 
	}
...
void drawPixel(multiboot_info_t *mbi, int x, int y, int r, int g, int b) {//Set a specific pixel on screen to white
	unsigned char *video = (unsigned char *)mbi->framebuffer_addr;
...
void drawLine(multiboot_info_t *mbi, int x1, int y1, int x2, int y2, int r, int g, int b){//Draw a line on the screen
