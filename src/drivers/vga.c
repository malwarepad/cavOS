...
void drawRect(multiboot_info_t *mbi, int x, int y, int w, int h, int r, int g,
              int b) { // Draw a filled rectangle
  int            i, j;
  unsigned char *video = (unsigned char *)mbi->framebuffer_addr;
  unsigned int   offset =
      (x + y * mbi->framebuffer_width) *
...
void drawText(multiboot_info_t *mbi,
              int charnum) { // draw a single character using a 8*8 pattern
  ...
  if (height == mbi->framebuffer_height) { // scrolling screen by clearing
    clearScreen(mbi);
  }
      ...
      for (i = 0; i < 8; i++) {
        if (font[charnum * 8 + k] & (0x01 << i)) {
          for (w = 2 * (8 - i); w < 2 * (8 - i) + 2; w++) {
            for (h = 2 * k; h < 2 * k + 2; h++) {
              drawPixel(mbi, w + width, h + height, textcolor[0], textcolor[1],
                        textcolor[2]);
            }
          }
        }
...
void drawPixel(multiboot_info_t *mbi, int x, int y, int r, int g,
...
void drawLine(multiboot_info_t *mbi, int x1, int y1, int x2, int y2, int r,
              int g, int b) { // Draw a line on the screen
