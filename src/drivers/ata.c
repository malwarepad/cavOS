...
void read_sectors_ATA_PIO(uint8_t *target_address, uint32_t LBA,
  ...
  port_byte_out(ATA_MASTER_BASE + ATA_REG_HDDEVSEL,
                ATA_MASTER | ((LBA >> 24) & 0xF));
  // Send the amount of sectors we want
  port_byte_out(ATA_MASTER_BASE + ATA_REG_SECCOUNT0, sector_count);
  // Send LBA, 8 bits at a time!
  port_byte_out(ATA_MASTER_BASE + ATA_REG_LBA0, (uint8_t)LBA);
  port_byte_out(ATA_MASTER_BASE + ATA_REG_LBA1, (uint8_t)(LBA >> 8));
  port_byte_out(ATA_MASTER_BASE + ATA_REG_LBA2, (uint8_t)(LBA >> 16));
  // Read already!
  port_byte_out(ATA_MASTER_BASE + ATA_REG_COMMAND, 0x20);
